...
arima拟合flow效果:  
![image](images_for_readme/4.png)  

kalman拟合flow效果：  
![image](images_for_readme/5.png)  

arima拟合speed效果：  
![image](images_for_readme/6.png)   

kalman拟合speed效果：  
![image](images_for_readme/7.png)   
...
4.4.2建模过程
逻辑回归算法分类步骤： 
	对输入的样本计算预测的输出，形如Z=K^T X+b，其中Z,K 和 X 均为矩阵，X为flow、speed等特征值，K为预测出的权值
	对输出 Z 中的每个 𝑧 计算 sigmoid 函数，将输出结果映射到 0/1，其中 sigmoid 函数定义为:
...
对于样本集D={(x_1^((1) )ⓜ,x_2^((1) )ⓜ,...ⓜ,x_n^((1) )ⓜ,y_1 )ⓜ,(x_1^((2) )ⓜ,x_2^((2) )ⓜ,...ⓜ,x_n^((2) )ⓜ,y_2 )ⓜ,...ⓜ,(x_1^((m) )ⓜ,x_2^((m) )ⓜ,...ⓜ,x_n^((m) )ⓜ,y_m ) }，其中m表示共有m个样本，n表示有n个特征值，y_i (i=1ⓜ,2ⓜ,...ⓜ,m)表示样本类别，值为{c_1ⓜ,c_2ⓜ,...ⓜ,c_k }。
朴素贝叶斯分类的基本公式：
P(Y=C_k|X=x)=\cfrac{P(Y=C_k)\prod_{j=1}^nP(X_j=x_j|Y=C_k)}{\sum_kP(Y=C_k)\prod_{j=1}^nP(X_j=x_j|Y=C_k)} 
特征的可能性假设为高斯分布：
P(x_i|y)=\cfrac{1}{\sqrt{2\pi\sigma_y^2}exp(-\cfrac{(x_i-\mu_y)^2}{2\sigma_y^2})} 
参数σ_y和μ_y使用最大似然法估计。
对于给出的待分类项，求解在此项出现的条件下各个类别出现的概率，此分类项的类别是概率最大的类别。也就是说，如果P(y_k |x)=max{P(y_1 |x)ⓜ,P(y_2 |x)ⓜ,…ⓜ,P(y_n |x) }，那么x的类别是y_k。
4.5.3建模结果
输出模型评价指标如下：
AUC	0.9241
Accuracy	0.8619
...
从以上建模结果中展示的、测试集中的评估指标来对比分析的模型质量。
	卡尔曼滤波	条件逻辑回归	非条件逻辑回归	朴素贝叶斯
AUC	/	0.877	0.805/0.817（非逐步/逐步）	0.9241
Accuracy	0.9186	0.762	0.829	0.8619
Precision	0.9987	0.749	0.949	0.9982
Recall	0.9196	0.948	0.855	0.8619
F1 Score	0.9575	0.837	0.900	0.9243
