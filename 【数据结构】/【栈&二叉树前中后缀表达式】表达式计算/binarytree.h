...
public:
	...
	int Brother(TreeNode<T>* location, TreeNode<T>* NOT_SHOW = NULL);//输出location节点的所有伦理兄弟
...
{
	if (start != NULL)//递归思路：先左子节点，后中间节点，最后右子节点
	{
...
{
	if (start != NULL)//递归思路：先中间节点，后左子节点，最后右子节点
	{
		T show = start->GetData();
		if(show!= NOT_SHOW_1&& show != NOT_SHOW_2)
		    cout << show<<' ';
...
{
	if (start != NULL)//递归思路：先左子节点，后右子节点，最后中间节点
	{
		T show = start->GetData();
...
int BinaryTree<T>::Brother(TreeNode<T>* location, TreeNode<T>* NOT_SHOW)//输出location节点的所有伦理兄弟，即输出其所在的右子树一枝上的除该节点外所有的节点
{
	...
	{
		while (location->GetRight() != NULL)//遍历该枝右分支
		{
