...
class GDConv(nn.Module):
    """
    GDConv(
    (depthwise): Conv2d(512, 512, kernel_size=(7, 7), stride=(1, 1), groups=512, bias=False)
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        self.conv = nn.Sequential(*layers)  # InvertedResidual的网络结构
...
class MobileFaceNet(nn.Module):
    def __init__(self, width_mult=1.0, inverted_residual_setting=None, round_nearest=8):
        ...
        input_channel = 64  # 输入通道数
        last_channel = 512  # 输出通道数
        ...
        # 判断inverted_residual_setting的格式是否正确
        ...
        self.conv_first = nn.Conv2d(3, 3, kernel_size=3)  # Conv2d(3, 3, kernel_size=(3, 3), stride=(1, 1))
        ...
        for t, c, n, s in inverted_residual_setting:
            output_channel = _make_divisible(c * width_mult, round_nearest)  # 每一层的输出通道数
            for i in range(n):
        ...
        self.bn = nn.BatchNorm2d(
            512)  # BatchNorm2d(512, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
    ...
    def forward(self, x):
        ...
        # print('conv1', x.shape)
        x = self.dw_conv(x)  # [4, 64, 112, 112]
        # print('dw_conv', x.shape)
        x = self.features(x)  # [4, 128, 7, 7]
        # print('feature', x.shape)
        x = self.conv2(x)  # [4, 512, 7, 7]
        # print('conv2', x.shape)
        x = self.gdconv(x)  # [4, 512, 1, 1]
        # print('gdconv', x.shape)
        x = self.bn(x)  # [4, 512, 1, 1]
        # print('bn', x.shape)
        x = x.view(x.size(0), -1)  # [4, 512]
        # print('view', x.shape)
