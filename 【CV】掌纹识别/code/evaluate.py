...
def parseList(root):
    ...
    nameRs = []  # 右手图
    folds = []  # 每800条数据为一组
    flags = []  # 两张图像是不是一个人的(1是；-1不是)
    for i, p in enumerate(pairs):
        p = p.split('\t')
        # 两张图像属于同一个人
        # 73
        # /root/autodl-tmp/generation/ROI/037_2_m_r_08_ROI.jpeg
        # /root/autodl-tmp/generation/ROI/037_1_m_r_06_ROI.jpeg
        if len(p) == 3:
            nameL = p[1]
            nameR = p[2]
            fold = i // 800
        ...
        # 两张图像属于不同人
        # 316
        # /root/autodl-tmp/generation/ROI/159_2_m_l_01_ROI.jpeg
        # 64
        # /root/autodl-tmp/generation/ROI/033_2_m_l_03_ROI.jpeg
        elif len(p) == 4:
            nameL = p[1]
            nameR = p[3]
            fold = i // 800
        ...
        flags.append(flag)
    # print(nameLs)
    print('nameLs:', len(nameLs))
    print('nameRs:', len(nameRs))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', type=str,
                        default='/root/autodl-tmp/palm/my_pth/MobileViT/MobileViT_20230608_224728/best.pth',
                        help='The path pf save model')
    parser.add_argument('--feature_save_dir', type=str, default='/root/autodl-tmp/palm/my_pth/eval_result.mat',
