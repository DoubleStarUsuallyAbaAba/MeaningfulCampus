...
class SequenceFolder(torch.utils.data.Dataset):
    ...
    def __init__(self, root, seed=None, ttype='scene_list.txt', sequence_length=3, sequence_gap = 1, transform=None, height = 240, width = 320):
        np.random.seed(seed)
        random.seed(seed)
        self.root = Path(root)
    ...
    def crawl_folders(self, sequence_length,sequence_gap):
        ...
        for scene in self.scenes:
            ...
            isc+=1
            frames = os.listdir(os.path.join(scene, "image"))
            frames = [int(os.path.splitext(frame)[0]) for frame in frames if frame != '.ipynb_checkpoints']
            frames =  sorted(frames)
            poses = [np.loadtxt(os.path.join(scene, "pose", "{:0>7}.txt".format(frame))) for frame in frames]


            with open(os.path.join(scene, "intrinsics.txt")) as info_f:
            ...
            for i in range(len(frames)):
                ...
                img = os.path.join(scene, "image", "{:0>7}.jpg".format(i))
                depth = os.path.join(scene, "depth", "{:0>7}.png".format(i))
                ...
                for j in shifts:
                    ...
                    sample['ref_imgs'].append(os.path.join(scene, "image", "{:0>7}.jpg".format(j)))
                    sample['ref_depths'].append(os.path.join(scene, "depth", "{:0>7}.png".format(j)))
